...
export default function RecipeCarousel({ calories }) {
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        const url = calories 
          ? `http://84.247.139.124:3001/recipes/suggested?maxCalories=${calories}`
          : 'http://84.247.139.124:3001/recipes/suggested';
            ...
            return (
                <Link
                    ...
                    style={{
                        ...
                        textDecoration: 'none',
                        backgroundImage: `url(${`http://84.247.139.124:3001/${recipe.imagePath.replace(/\\/g, '/')}`})`,
