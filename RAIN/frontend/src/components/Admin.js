...
function AdminPanel() {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const res = await fetch('http://84.247.139.124:3001/users', {
        credentials: 'include'
      });
  ...
  const fetchExercises = async () => {
    try {
      setLoading(true);
      const res = await fetch('http://84.247.139.124:3001/exercises', {
        credentials: 'include'
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const deleteExercises = async (id) =>{
    try {
      setLoading(true);
      const res = await fetch(`http://84.247.139.124:3001/exercises/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      if (!res.ok) {
  ...
  const deleteUser = async (id) =>{
    try {
      setLoading(true);
      const res = await fetch(`http://84.247.139.124:3001/users/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      if (!res.ok) {
